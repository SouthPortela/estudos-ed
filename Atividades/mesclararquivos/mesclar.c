...
// BUSCA BINARIA
// ==============================
static int busca_binaria(char *Palavra, int Esquerda, int Direita, char *vetor){
    int Meio;
    if (Esquerda == Direita-1)
        return Direita;
    else {
        Meio = (Esquerda + Direita) / 2;
        if (strcmp(Palavra, vetor[Meio])){
            busca_binaria(&Palavra, Esquerda, Meio, vetor);
        } else {
            busca_binaria(&Palavra, Esquerda, Meio, vetor);
        }
...
int main(int argc, char *argv[]){
                ...
                while(token != NULL){
                    ...
                    if(strlen(token)>0 && isalpha(token[0])){
                        ...
                        if(indice>=0 && indice<26){
                            inserirOuIncrementar(&Alfabeto[indice], token);
                        }
                    }
                    token = strtok(NULL,";\n");
                }
    ...
    for(int i=0;i<26;i++){
        if(Alfabeto[i] != NULL){
            ...
            imprimirListaReversa(Alfabeto[i]);
        }
    }
    return 0;
}
